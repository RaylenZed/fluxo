...
# ============================================================
# Installs Mihomo (proxy core), Fluxo (web config workbench), or both
...
MIHOMO_GZ="${MIHOMO_GZ:-}"
INSTALL_MODE="${FLUXO_INSTALL_MODE:-full}"   # full | mihomo | web
APPLY_MODE="${FLUXO_APPLY_MODE:-manual}"     # manual | managed
...
want_mihomo() {
  [[ "$INSTALL_MODE" == "full" || "$INSTALL_MODE" == "mihomo" ]]
}

want_fluxo() {
  [[ "$INSTALL_MODE" == "full" || "$INSTALL_MODE" == "web" ]]
}
...
# Managed by fluxo-cli/systemd. Fluxo Web exports YAML unless managed mode is enabled.
...
install_fluxo_cli_only() {
  ...
  log_warn "Failed to download fluxo-cli from ${cli_url}"
  log_detail "Mihomo itself is installed; you can download fluxo-cli later."
}
...
[Unit]
Description=Fluxo - Mihomo Config Workbench
...
Environment=FLUXO_DEFAULT_APPLY_MODE=${APPLY_MODE}
...
EOF
  ...
  want_mihomo && systemctl enable mihomo.service
  want_fluxo && systemctl enable fluxo.service
  want_fluxo && systemctl enable fluxo-web.service
...
configure_firewall() {
  ...
  local ports=()
  want_fluxo && ports+=("${WEB_PORT}/tcp")
  want_mihomo && ports+=("7890/tcp")

  if [[ ${#ports[@]} -eq 0 ]]; then
    log_detail "No network services selected — skipping"
    return 0
  fi

  if command -v ufw &>/dev/null && ufw status | grep -q "active"; then
    log_detail "ufw detected and active"
    if confirm "Open selected service ports (${ports[*]}) in ufw?"; then
      want_fluxo && ufw allow "$WEB_PORT/tcp" comment "fluxo UI" &>/dev/null
      want_mihomo && ufw allow "7890/tcp" comment "mihomo proxy" &>/dev/null
      ufw reload &>/dev/null
      log_info "Firewall rules added"
    fi
  elif command -v firewall-cmd &>/dev/null && firewall-cmd --state &>/dev/null; then
    log_detail "firewalld detected"
    if confirm "Open selected service ports (${ports[*]}) in firewalld?"; then
      want_fluxo && firewall-cmd --permanent --add-port="${WEB_PORT}/tcp" &>/dev/null
      want_mihomo && firewall-cmd --permanent --add-port="7890/tcp" &>/dev/null
...
show_summary() {
  ...
  if want_fluxo; then
    ...
    echo -e "  Apply mode       : ${CYAN}${APPLY_MODE}${NC}"
    if [[ "$APPLY_MODE" == "manual" ]]; then
      echo -e "  Generated YAML   : ${CYAN}${DATA_DIR}/generated.yaml${NC}"
    else
      echo -e "  Managed config   : ${CYAN}${MIHOMO_CONFIG_DIR}/config.yaml${NC}"
    fi
    echo -e ""
  fi
  if want_mihomo; then
  ...
  command -v fluxo-cli &>/dev/null && echo -e "  ${CYAN}fluxo-cli${NC}                         — interactive CLI (Mihomo management)"
  command -v fluxo-cli &>/dev/null && echo -e "  ${CYAN}fluxo-cli status${NC}                  — quick status"
  command -v fluxo-cli &>/dev/null && echo -e "  ${CYAN}fluxo-cli test${NC}                    — network connectivity test"
  want_mihomo && echo -e "  ${CYAN}systemctl status mihomo${NC}           — core status"
  want_fluxo && echo -e "  ${CYAN}systemctl status fluxo${NC}            — Fluxo API status"
  want_fluxo && echo -e "  ${CYAN}systemctl status fluxo-web${NC}        — Fluxo Web status"
  want_mihomo && echo -e "  ${CYAN}journalctl -fu mihomo${NC}             — core logs (live)"
  want_fluxo && echo -e "  ${CYAN}journalctl -fu fluxo${NC}              — Fluxo API logs (live)"
  want_fluxo && echo -e "  ${CYAN}journalctl -fu fluxo-web${NC}          — Fluxo Web logs (live)"
  ...
  echo -e ""
  if want_mihomo; then
    echo -e "${YELLOW}  Mihomo config file: ${MIHOMO_CONFIG_DIR}/config.yaml${NC}"
    echo -e "${YELLOW}  Reload Mihomo after importing config:${NC}"
...
run_installer() {
  ...
  echo -e "  Fluxo apply mode : ${CYAN}${APPLY_MODE}${NC}"
  want_mihomo && echo -e "  Mihomo version   : ${CYAN}${MIHOMO_VERSION}${NC}"
  want_fluxo && echo -e "  Web UI port      : ${CYAN}${WEB_PORT}${NC}"
  want_fluxo && echo -e "  API server port  : ${CYAN}${SERVER_PORT}${NC}"
  want_fluxo && echo -e "  Install dir      : ${CYAN}${INSTALL_DIR}${NC}"
  want_fluxo && echo -e "  Data dir         : ${CYAN}${DATA_DIR}${NC}"
  want_mihomo && echo -e "  Mihomo archive   : ${CYAN}${MIHOMO_GZ:-（download）}${NC}"
  echo -e "  GitHub proxy     : ${CYAN}${GH_PROXY:-（none）}${NC}"
  want_fluxo && echo -e "  npm registry     : ${CYAN}${NPM_REGISTRY:-（default）}${NC}"
  echo -e ""
...
show_help() {
  echo "Usage: curl -fsSL https://fluxo.click | sudo bash [--upgrade] [--managed|--manual] [--mihomo-only|--web-only]"
  echo ""
  echo "Modes:"
  echo "  --manual        Fluxo only generates/exports YAML (default)"
  echo "  --managed       Fluxo writes config and reloads Mihomo (legacy integrated mode)"
  echo "  --mihomo-only   Install Mihomo + fluxo-cli only"
  echo "  --web-only      Install Fluxo Web/API only"
  echo ""
  echo "Environment variables:"
  echo "  FLUXO_INSTALL_MODE full | mihomo | web (default: full)"
  echo "  FLUXO_APPLY_MODE   manual | managed (default: manual)"
  ...
  echo ""
  echo "Examples:"
  echo "  # Split install: Mihomo is managed by CLI; Web exports YAML"
  echo "  curl -fsSL https://fluxo.click/cn | bash -s -- --upgrade"
  echo ""
  echo "  # Mihomo core only"
  echo "  curl -fsSL https://fluxo.click/cn | bash -s -- --mihomo-only"
  echo ""
  echo "  # Fluxo Web only"
  echo "  curl -fsSL https://fluxo.click/cn | bash -s -- --web-only"
  echo ""
  echo "  # Legacy integrated behavior"
  echo "  curl -fsSL https://fluxo.click/cn | bash -s -- --managed"
  echo ""
...
for arg in "$@"; do
  case "$arg" in
    ...
    --upgrade|upgrade) PIPE_MODE=true ;;
    --help|-h|help) COMMAND="help" ;;
    --manual|manual) APPLY_MODE="manual" ;;
    --managed|managed) APPLY_MODE="managed" ;;
    --mihomo-only|--core-only|mihomo-only)
      INSTALL_MODE="mihomo"
      APPLY_MODE="manual"
      ;;
    --web-only|web-only)
...
case "$INSTALL_MODE" in
  full|mihomo|web) ;;
  *) die "Invalid FLUXO_INSTALL_MODE: ${INSTALL_MODE}. Use full, mihomo, or web." ;;
esac
case "$APPLY_MODE" in
  manual|managed) ;;
  *) die "Invalid FLUXO_APPLY_MODE: ${APPLY_MODE}. Use manual or managed." ;;
esac
